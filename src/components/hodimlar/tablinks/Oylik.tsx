...
// import { deleteSelected } from '../../../redux/hodimlarSlice/oylikSlice';
import { useNavigate } from 'react-router-dom';
import { deleteSelected } from '@/store/hodimlarSlice/oylikSlice';
...
const Oylik: React.FC<PropsType> = ({ count, setCount }) => {
	const dispatch = useDispatch();

	// Fetch the items from the Redux store
	const items = useSelector((state: RootState) => state.oylik.items); // Adapt to your state structure
	const [selectedRows, setSelectedRows] = useState<number[]>([]); // Holds selected row IDs
	const [selectedData, setSelectedData] = useState<Item[]>([]); // Holds selected data
